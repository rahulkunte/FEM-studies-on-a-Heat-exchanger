...
% L3(1,1) = 0;
% for i=2:n+1
% L3(i,1)=(L3(i-1,1)+L1/(n));
% end
% % L3;
% % plot(L3,T_sub)
% % title('8 Sub-Sections')
% % xlabel('Length(m)')
...
% grid on
% syms F y
% F = a1*exp(-((y-b1)/c1)^2) + a2*exp(-((y-b2)/c2)^2) + a3*exp(-((y-b3)/c3)^2) + a4*exp(-((y-b4)/c4)^2) + a5*exp(-((y-b5)/c5)^2) + a6*exp(-((y-b6)/c6)^2) + a7*exp(-((y-b7)/c7)^2) + a8*exp(-((y-b8)/c8)^2) ;
% c = int(F,y,330,Th(n+1));
% c = (1/(Th(n+1)-330))*c;
% eh = ((mh*4.8287)*(330-Th(n+1,1)))/(min(mh*4.8287,mc*4.187)*(330-286.22551));
% ec = ((mc*4.187)*(Tc(1,1)-280))/(min(mh*4.8287,mc*4.187)*(330-286.22551));
